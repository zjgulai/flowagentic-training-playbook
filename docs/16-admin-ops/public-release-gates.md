...
- 构建与测试 Job 只有只读权限；它把已验证的 mike 静态树同时保存为 `gh-pages` 留档和标准 Pages 制品。受环境审批保护的写权限 Job 先以单个 Git 提交更新 `latest` 与版本目录，再通过 `actions/deploy-pages` 部署同一制品，并在部署成功后执行只读 HTTP 冒烟。
